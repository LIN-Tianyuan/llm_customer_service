...
# 默认的config.yml内容
DEFAULT_CONFIG_CONTENT = '''# Lemon AI 配置文件
# LLM驱动的对话系统配置
...
def init_command(
    ...
    示例:
        lemon init                    # 在当前目录初始化
        lemon init --path ./my_bot    # 在指定目录初始化
        lemon init --template basic   # 使用basic模板
    """
    click.echo("欢迎使用 Lemon AI!\n")
    click.echo("正在创建新项目...\n")
    ...
    try:
        ...
        click.echo("  3. 运行 'lemon train' 训练模型")
        click.echo("  4. 运行 'lemon shell' 测试对话")
        click.echo("")
