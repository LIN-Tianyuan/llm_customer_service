...
from lemon_ai.agent.message_processor import (
    MessageProcessor,
    ProcessorConfig,
    MessageResponse,
)
from lemon_ai.agent.actions import register_action, Action
from lemon_ai.agent.graph import (
    get_message_processing_graph,
    create_initial_state,
)
from lemon_ai.core.tracker import DialogueStateTracker
from lemon_ai.core.domain import Domain
from lemon_ai.core.stores import create_tracker_store, TrackerStore
from lemon_ai.dialogue_understanding.flow import FlowsList, FlowLoader
from lemon_ai.dialogue_understanding.generator import LLMCommandGenerator
from lemon_ai.dialogue_understanding.processor import CommandProcessor
from lemon_ai.policies import PolicyEnsemble, FlowPolicy, EnterpriseSearchPolicy
from lemon_ai.shared.yaml_loader import read_yaml_file
from lemon_ai.shared.config import AtguiguConfig, LLMConfig
...
class Agent:
    ...
    def load(
        ...
        """
        import tempfile
        from lemon_ai.training.model_storage import (
        ...
        from lemon_ai.shared.config import EndpointsConfig
        ...
        if config_path.exists():
            ...
            if config_data:
                ...
                if "retrieval" in config_data:
                    from lemon_ai.shared.config import RetrievalConfig
        ...
        if llm_config:
            from lemon_ai.dialogue_understanding.generator import (
                LLMCommandGenerator,
                LLMGeneratorConfig,
            )
        ...
        # 创建策略
        from lemon_ai.policies import EnterpriseSearchPolicyConfig
        ...
        if retriever_class_path:
            try:
                from lemon_ai.retrieval import create_retriever
        ...
        if nlg_config and nlg_config.rephrase_enabled:
            try:
                from lemon_ai.nlg import ResponseRephraser, RephraserConfig, TemplateNLG
                ...
                if rephrase_llm_config:
                    ...
                    # 创建LLM客户端
                    from lemon_ai.shared.llm import create_llm_client
